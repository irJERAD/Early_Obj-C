...
#pragma mark - Application delegrate callbacks

- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    // Attmpe to load an existing to-do dataset from an array stored to disk.
    NSArray *plist = [NSArray arrayWithContentsOfFile:docPath()];
    if (plist) {
        // if there was a dataset available, copy it into our istance variable
    ...
    if ([tasks count] == 0) {
        ...
        [tasks addObject:@"Call Ashley"];
        [tasks addObject:@"Express Self and Emotions"];
        [tasks addObject:@"Work towards being a better person"];
        [tasks addObject:@"Focus: Work; Focus: Work; Imrpove, Focus: work"];
    }
    */
    // Create and configure the UIWindow instance
    // A CGRect is a strict with an origin (x, y) and size, height)
...
{
    // To improve performane, we reconfigure cells in memory
    ...
    if(!c) {
        // ...and only allocate a new cell if non are available
...
{
    // This method is only called in iOS versions piror to 4.0
